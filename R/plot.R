...
#' plotting purposes. This functionality is useful where a discretization with 2nd order Finite Element is applied. This parameter can be specified only when a planar mesh (\code{mesh.2D}) object is plotted.
...
#'If the \code{mesh} of the \code{FEMbasis} component is of class \code{mesh.2D} both the 3rd axis and the color represent the value of the coefficients for the Finite Element basis expansion (\code{coeff} component of the \code{FEM} object). If the \code{mesh} is of class \code{mesh.2.5D} or \code{mesh.3D}, the color of each triangle or tetrahedron represent the mean value of the coefficients for the Finite Element basis expansion(\code{coeff}).
...
#' 
#' @param x A \code{mesh.2D} object defining the triangular mesh, as generated by \code{create.mesh.2D} or \code{refine.mesh.2D}.
#' @param ... Arguments representing graphical options to be passed to \link[graphics]{par}.
#' @description Plot a mesh mesh.2D object, generated by \code{create.mesh.2D} or \code{refine.mesh.2D}. Circles indicate the mesh nodes.
#' @name plot.mesh.2D
...
#'
#' @param x A \code{mesh.2.5D} object generated by \code{create.mesh.2.5D} or \code{second.order.mesh.2.5D}.
#' @param ... Arguments representing graphical options to be passed to \link[graphics]{par}.
...
#'
#' @param x A \code{mesh.3D} object generated by \code{create.mesh.3D} or \code{second.order.mesh.3D}.
#' @param ... Arguments representing graphical options to be passed to \link[graphics]{par}.
...
plot.mesh.3D<-function(x,...){
 ...
 #' @param ... Arguments representing  graphical options to be passed to \link[rgl]{plot3d}.
 #' @description Image plot of a \code{FEM} object, generated by the function \code{FEM} or returned by \code{smooth.FEM.basis} can be visualized through an image plot. Only planar mesh (\code{mesh2D}) objects can be plotted with this method.
