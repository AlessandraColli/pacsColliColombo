...
template<int NDIME, int NDIMP>
Box<NDIME, NDIMP>::Box(std::vector<Real> const & coord) {
	x_.resize(2*NDIMP); //multiply 2 for min, max
	for(int i = 0; i < 2*NDIMP; ++i) {
		x_[i] = coord[i];
	}
...
Box<NDIME, NDIMP>::Box(Element<NNODES,NDIME,NDIMP> const & element) {
	// if(typeid(NDIMP) != typeid(2)) {
	 //    std::cout << std::endl << std::endl;
	 //    std::cout << "error! Box<NDIME, NDIMP>::Box(Element<NNODES,NDIME,NDIMP> const & element) : bad template parameter" << std::endl << std::endl;
	...
	if(NDIME == 2 && NDIMP == 2) {
  	...
  	} else if(NDIME == 3 && NDIMP == 3) { //Tetrahedron has 4 nodes
  		x_.resize(3*NDIMP); //6
...
template<int NDIME, int NDIMP>
void Box<NDIME, NDIMP>::set(std::vector<Real> const & data) {
...
template<int NDIME, int NDIMP>
void Box<NDIME, NDIMP>::print(std::ostream & out) const {
	out << "Min_Point:  ( - ";
