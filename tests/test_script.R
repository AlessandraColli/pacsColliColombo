...
data_exact2 = sin(pi*xobs)

# Perturbed data - locations different from nodes
data2 = data_exact2 + rnorm(n = length(xobs), sd = 0.1)
...
plot(FEM(output_CPP2$fit.FEM$coeff[,which.min(output_CPP2$GCV)],FEMbasis))
...
lambda2=seq(1.8*10^-5,2*10^-5,by=10^-7)
...
plot(FEM(smooth_areal2$fit.FEM$coeff[,which.min(smooth_areal2$GCV)]+W%*%smooth_areal2$beta[,which.min(smooth_areal2$GCV)],FEMbasis))
